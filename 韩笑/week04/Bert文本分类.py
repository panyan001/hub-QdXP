...
# 加载和预处理数据
dataset_df = pd.read_csv("新闻标题.csv", sep=",", header=None, encoding="utf-8")
...
model = BertForSequenceClassification.from_pretrained("bert-base-chinese", num_labels=len(lbl.classes_))
...
# 2. 打印详细分类报告
print("\n 分类报告:")
target_names = lbl.classes_  # ['体育', '财经', ...]
...
def predict_text(text: str):
    ...
    inputs = {k: v.to(model.device) for k, v in inputs.items()}  # 如果用 GPU
    ...
    return lbl.inverse_transform([pred_class])[0]
...
for sent in test_sentences:
    pred_label = predict_text(sent)
    print(f"文本: '{sent}' → 预测类别: {pred_label}")
