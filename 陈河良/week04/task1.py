...
# 加载和预处理数据
dataset_df = pd.read_csv("D:\\AILearning\\datasets\\weibo_senti_100k\\weibo_senti_100k.csv", sep=",", header=None)

print(type(dataset_df))
print(dataset_df)

# 统计每个类别的数量
label_counts = dataset_df[0].value_counts()
print("原始类别分布:")
print(label_counts)
# print(dataset_df.columns if hasattr(dataset_df, 'columns') else "Not a DataFrame")

# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换前500个标签，得到数字标签
labels = lbl.fit_transform(dataset_df[0].values[1:10000])
# 提取前500个文本内容
texts = list(dataset_df[1].values[1:10000])
