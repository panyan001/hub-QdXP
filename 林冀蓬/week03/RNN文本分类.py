...
def create_word_table(dataset: pd.DataFrame):
...
class RNNClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc(hidden_state[-1]) #取最后一个时间步的输出
        return out

def rnn_train(model, optimizer, loss_fn, num_epochs, dataloader):
    ...
    torch.save(model.state_dict(), "RNNClassifier.pth")
...
if __name__ == "__main__":
    ...
    rnn_dataset = CharRNNDataset(dataset[0].tolist(), numerical_labels, char_to_index, max_len)
    rnn_dataloader = DataLoader(rnn_dataset, batch_size=32, shuffle=True)

    rnn_train(model, optimizer, loss_fn, epochs_num, rnn_dataloader)

    model.load_state_dict(torch.load("RNNClassifier.pth"))
    model.eval()
