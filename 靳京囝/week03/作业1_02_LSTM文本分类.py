...
class LSTMClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, num_layers=1, dropout=0.1):
        ...
        self.lstm = nn.LSTM(
            ...
            batch_first=True,
            dropout=dropout if num_layers > 1 else 0, # 仅多层时启用dropout
            bidirectional=True,
        )
    ...
    def forward(self, x):
        ...
        # hidden_state维度：[num_layers*2, batch_size, hidden_dim]
        # 取最后一层的正向和反向隐藏状态拼接
        # - 正向最后一层：hidden_state[-2, :, :]
        # - 反向最后一层：hidden_state[-1, :, :]
...
model = LSTMClassifier(vocab_size, embedding_dim, hidden_dim, output_dim, num_layers=num_layers, dropout=dropout)
